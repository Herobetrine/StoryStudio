...
`npm ci --omit=dev`；不带 lockfile 的 npm 打包产物使用
`npm install --omit=dev --no-audit --no-fund --package-lock=false`。脚本会记录当前生效的依赖清单摘要：
优先使用 `npm-shrinkwrap.json`，其次是 `package-lock.json`，两者都不存在时使用
`package.json`。对应清单发生变化时会自动重装，不会把仍可解析但已经过期的
`node_modules` 误判为最新状态。
看到服务地址后，在浏览器打开：
...
npm tarball 默认不包含 lockfile。解包后改用：
...
12. 内容修改约 700 毫秒后自动保存。顶部状态显示“已保存”后即可刷新或关闭页面；如果手机在这 700 毫秒内切到后台或关闭标签页，浏览器本地恢复草稿会承接尚未完成的保存。

桌面端采用章节、正文、检查器三栏布局；在手机上，顶部的章节图标和检查器图标会打开左右抽屉。

### 关闭页面或切到后台时的草稿恢复

正文、章纲、卷纲和作品设定一旦进入“待保存”，StoryStudio 会立即把本次改动按作品和章节写入浏览器 `localStorage`，不等待 700 毫秒自动保存计时结束。页面进入后台或触发 `pagehide` 时会再写一次；如果本次只有章节字段变化，还会尝试用 `keepalive` 请求把正文等修改送到服务端。正常保存成功，或检测到同样内容已经进入权威数据后，本地恢复草稿会自动删除。

重新打开同一浏览器配置、同一访问地址和同一章节时，系统会先读取服务端权威版本，再处理本地草稿。恢复草稿按每个浏览器标签页的 writer 身份分别保存；复制标签页或同时打开多个窗口时，活跃页面会通过浏览器消息协调出不同身份，避免互相覆盖同一个草稿 key：

- 同一标签页刷新后，writer 身份和作品、章节、相关卷的权威版本都仍与草稿记录一致时，系统直接恢复改动并重新进入自动保存。
- 来自其他标签页、复制标签页、上次浏览器会话或旧版单 key 格式的草稿，即使权威版本未变化也会先询问，再由作者决定是否应用。
- 服务端版本已经变化时，系统不会静默覆盖，而是询问是否把本地草稿中的改动字段重新应用到当前版本。确认后应检查同字段是否存在他人或其他窗口的新修改；取消则保留本地草稿，下次打开本章时再次询问。
- 本地恢复草稿是浏览器侧的临时兜底，不随作品导出，也不能替代完整 `data` 备份。无痕模式、清理站点数据、更换浏览器、端口或访问地址都可能使原草稿不可见。
...
角色卡、世界书和 Prompt Profile 在导入、读取资源详情以及随作品导出时，都会递归清理明确的凭据字段，包括 API key（含 Provider 前后缀及 Unicode 组合字符变体）、Authorization/API-key header、常见认证/HTTP/请求/自定义/原始/Provider/连接类 header 容器、access/refresh/session/bearer/auth token、已知 Provider token、client secret/password、带前缀的 password/passphrase、session cookie、带业务前缀的 private/signing/secret/encryption key、webhook secret 和 credential。清理覆盖嵌套 `extensions`、世界书条目扩展和保留的 `source.raw`；检测到的字段路径记录在 `source.removedSensitiveFields`，Prompt Profile 也会在兼容性页显示相应结果。即使旧资源文件曾把这些字段写入磁盘，资源 API 和后续作品导出也只返回清理后的内容。

所有资源类型都会移除明确的 header 容器；Prompt 预设还会移除 URL/endpoint、account/project、region、deployment 等模型连接信息，不会用导入文件替换模型设置。为避免误删创作数据，普通名为 `token`、`secret` 的叙事元数据、普通 `*_key`、角色卡或世界书中的普通 URL/endpoint，以及 `tokenBudget`、`maxTokens` 等预算字段会保留；自定义扩展仍应避免用含糊字段存放真实凭据。
...
打开或刷新策划工作台时，前端会通过带 CSRF 保护的显式 `POST .../copilot/sessions/reconcile` 对账异常退出前留在 `generating` 的会话。它跳过当前进程仍在执行的请求，只把已经没有本机执行任务的旧会话恢复为可重试的中断状态，重复调用结果一致。会话列表和详情的 `GET` 保持纯读取，不会因为查看记录而修改磁盘状态。
...
7. 每次作者重新点击交接都会生成新的 command ID；只有同一次网络结果不确定的重试或精确重放复用原 ID。持久化 handoff V2 快照只保存来源 ID、方向坐标和摘要，完整内容保存在 run-scoped Artifact 中；同一 `POST .../workflow-runs` 重放会恢复缺失 Artifact、审批和步骤推进，不再调用 Provider，也不会建立重复 Workflow。对应的 `GET .../workflow-runs` 列表和详情严格只读，只展示中断状态，不以读取请求触发恢复或权威变化；若作品仍有待恢复 journal 或旧 Schema，GET 分别返回 `recovery_required` 或 `migration_required`，由后续认证写请求完成恢复/迁移。
...
作品、卷和章节分别使用 `project.version`、`volume.revision` 与 `chapter.revision`。两个浏览器窗口同时编辑同一内容时，服务端会拒绝过期版本；前端对作品、卷纲和章节尝试按字段进行三方合并，无法无损合并的同字段修改会明确显示冲突，不会静默覆盖。Workflow 刷新到更新权威快照时，尚未保存或正在提交的字段仍保留刷新前的比较基线和乐观锁令牌，必须先完成同样的冲突对账，不能把新 revision 当作旧标签页的覆盖许可。
...
`test:browser:install` 只需在首次运行或 Playwright 浏览器版本变化后执行。浏览器回归使用
独立临时数据目录和动态本机端口，覆盖多标签页并发保存、移动端生命周期 keepalive
与本地恢复草稿隔离，不读取正式 `data/`。这些测试命令面向完整源码仓库；npm tarball
有意只包含运行时文件，不包含 Playwright 配置、测试源码或浏览器开发依赖。
