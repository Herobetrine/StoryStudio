...
- `public/save-state.js`：乐观锁令牌、Dirty/Saving 批次、权威快照三方合并、冲突分类和 PATCH 投影等无副作用保存模型。
...
GET    /api/story-studio/projects/:projectId/chapters/:chapterId/authority
...
- `projectSavingPaths`、`chapterSavingPaths`、`volumeSavingFields`：已从 Dirty Set 暂移、但服务器尚未确认的在途保存字段；
...
保存循环依次处理作品级改动、当前卷纲改动和章节级改动。发请求前会从 Dirty Set 暂时移出本批字段，同时把它们登记到对应 Saving Set；保存期间产生的新输入继续留在 Dirty Set，当前请求成功后再处理下一轮。服务器快照合并、Workflow 权威刷新和浏览器恢复草稿都保护 `Dirty Set ∪ Saving Set`，因此并发刷新或保存冲突不能覆盖正在提交的本地内容。对这些被保留字段，浏览器同时保留刷新前的字段基线以及 `project.version`、`volume.revision`、`chapter.revision` 乐观锁令牌；普通自动保存和 `visibilitychange/pagehide` 的章节 keepalive 都使用这组基线令牌，必须先触发服务端 409 和三方对账，不能直接借用刷新后的 authority 覆盖另一个窗口的 Workflow 结果。只有与当前保存批次精确对应的成功响应，或用户已处理完冲突的远端快照，才会推进基线并采用服务端规范化后的最终值。

Workflow 刷新权威状态使用 `GET .../chapters/:chapterId/authority`。该接口复用只读项目/章节联合快照：读取前后校验项目与章节文件摘要、项目章节摘要索引和待恢复日志，读取期间发生变化时返回 `project_busy`，不会把两个独立请求的不同时点结果拼成一个版本。
...
`public/save-state.js` 使用 `public/core.js` 的字段投影原语，只检查本地 Dirty Set。若某字段的 `remote` 已偏离 `base`，且 `remote` 又不同于 `local`，该字段才是真冲突。不同字段的并发修改会自动合并：以 `remote` 为新底稿，只把本地脏字段覆盖其上。保存编排仍留在 `public/app.js`，纯模块不接触 DOM、网络、定时器或全局状态。
...
Copilot handoff 的创建中断遵守更窄的读写合同：Workflow 的两个 GET 读取入口只返回磁盘上已经存在的 run/Artifact，不调用 `resumeCopilotHandoff()`，也不通过 StoryStore 写锁恢复跨文件 journal 或执行 Schema 迁移。纯读 authority 先拒绝四类待恢复 journal，再验证当前 Schema 的项目、章节、项目索引和文件摘要；读取期间发生外部变化时返回 `project_busy`。标准恢复方式是客户端重放最初那个认证 `POST .../workflow-runs`；服务端用相同 command ID、request digest、authority 和 V2 坐标验证后，才补齐缺失方向、选择状态、run revision 与 receipt。run command POST 在处理命令前也会先调用已存 handoff 恢复对账。两条 POST 路径都不会重新调用 Provider。这样，页面刷新可以观察中断，但爬虫、预览、轮询或只读 API 不会因为一次 GET 改变持久状态。
...
`StoryStudioStore` 构造时会先创建空的 `data/migration-backups` 根并固定其目录身份；尚未发生迁移时其中没有项目备份内容。迁移先完整验证源项目、章节和所有资源引用，再在该根下创建原始字节 snapshot 与 SHA-256 manifest。配置根的既有祖先、备份根及后续路径组件都拒绝符号链接、Windows junction/reparse point 和越界 realpath；构造后的根或祖先替换也会因身份变化而 fail-closed。项目自带 `manifest.json` 位于 snapshot 内，不会与控制清单碰撞。备份复核后才持久化 `.pending-schema-migration.json`，其中同时绑定 manifest digest、完整基线和完整 V5 目标；发布顺序固定为逐章、最后 `project.json`。恢复前再次验证备份；未知、分叉、损坏或运行态依赖缺失均保留证据并 fail-closed。
...
- 生产依赖只包含 Express、Helmet、Lucide 静态图标、MiniSearch 和 `write-file-atomic`；
...
首次运行 Chromium 回归前执行 `npm run test:browser:install`。Node.js 套件覆盖
HTTP/Provider 合同、前端纯函数和文件 Store；Playwright 套件使用独立临时数据目录与动态
端口，覆盖保存中的跨标签页权威变化、移动端 `visibilitychange/pagehide` keepalive，以及
复制标签页 writer 身份和恢复草稿隔离。Fixture 直接装配 `createApp()` 与合成 Mock
Provider，不覆盖 `server.js` 和 PowerShell 启动器；进程入口、发布包及启动器依赖刷新由
Node.js release/launcher 合同测试独立验证。
